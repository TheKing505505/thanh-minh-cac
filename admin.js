...
/**
 * FIX: Ghi services qua app.js (__tmcSaveServices) để đồng bộ lên
 * Firestore collection "system_configs" — khách vãng lai (chưa đăng nhập)
 * cũng thấy thay đổi ngay lập tức, không chỉ riêng máy admin.
 */
function saveServices(svcs) {
  localStorage.setItem("tmc_services", JSON.stringify(svcs)); // cache tức thì cho UI admin
  if (typeof window.__tmcSaveServices === "function") {
    window.__tmcSaveServices(svcs).catch(e => console.error("saveServices Firestore:", e));
  }
}
...
function saveCustomCategories(cats) {
  ...
  if (typeof window.__tmcSaveCustomCategories === "function") {
    window.__tmcSaveCustomCategories(cats).catch(e => console.error("saveCustomCategories Firestore:", e));
  }
}

function getShopSettings() {
  if (typeof window.__tmcGetShopSettings === "function") return window.__tmcGetShopSettings();
...
function saveShopSettings(s) {
  ...
  if (typeof window.__tmcSaveShopSettings === "function") {
    window.__tmcSaveShopSettings(s).catch(e => console.error("saveShopSettings Firestore:", e));
  }
}
...
async function loadAdminData() {
  ...
  try {
    ...
    console.log(`TMC admin: tải được ${orders.length} đơn hàng, ${users.length} khách hàng`);
    renderAdminOrders(orders);
    renderAdminUsers(users);
    renderAdminServices();
  } catch (e) {
    console.error("TMC loadAdminData lỗi:", e.code || e.message, e);
    showNotif(`Không tải được dữ liệu admin: ${e.code || e.message}`, "error");
    // Hiển thị lỗi rõ ràng trong panel khách hàng để dễ chẩn đoán
    const userListEl = document.getElementById("adminUserList");
    if (userListEl) userListEl.innerHTML = `<div class="no-data"><p>❌ Lỗi tải khách hàng: ${e.code || e.message}</p><p style="font-size:0.72rem;color:rgba(240,230,255,0.4);margin-top:6px">Kiểm tra Firestore Rules collection "users"</p></div>`;
  }
}
