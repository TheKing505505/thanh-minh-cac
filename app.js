 ...
 * === CHANGELOG (BUG FIXES) ===
 * FIX-1 [PROFILE TRỐNG]:
 *   onAuthStateChanged luôn đọc lại fresh từ Firestore bằng getDoc(doc(db,"users", uid))
 *   và nếu snapshot không tồn tại thì thử query where("uid","==",uid) làm fallback.
 *   Đảm bảo currentUserData không bao giờ null khi user đã đăng nhập thành công.
 *
 * FIX-2 [BƠM TIỀN KHÔNG HOẠT ĐỘNG]:
 *   Mọi nơi ghi/đọc user document đều dùng uid (Firebase Auth UID) làm document ID,
 *   nhất quán với lúc setDoc khi đăng ký. Không dùng username làm doc ID.
 *   updateDoc(doc(db,"users", currentUser.uid), ...) – đường dẫn chính xác.
 *
 * FIX-3 [SỐ DƯ HIỂN THỊ RÕ]:
 *   profileCard render số dư nổi bật hơn.
 *   Sau mỗi thao tác trừ tiền (submitOrder, openBuyTitle), re-fetch currentUserData
 *   từ Firestore để sync số dư mới nhất về UI ngay lập tức.
 */
...
/**
 * FIX-1 & FIX-2: Hàm trung tâm để tải dữ liệu user từ Firestore.
 * Document ID luôn là Firebase Auth UID – nhất quán với lúc đăng ký.
 * Nếu getDoc trực tiếp theo UID thất bại (permissions hoặc doc chưa tạo),
 * fallback sang query where("uid","==",uid).
 */
async function fetchUserData(uid) {
  try {
    // Ưu tiên: đọc thẳng theo document ID = uid (nhanh nhất, O(1))
    const snap = await getDoc(doc(db, "users", uid));
    if (snap.exists()) return snap.data();
  } catch (e) {
    console.warn("TMC: getDoc by uid failed, thử query fallback:", e.code);
  }
  // Fallback: query theo field uid (dùng khi doc ID khác uid vì lý do legacy)
  try {
    const qSnap = await getDocs(query(collection(db, "users"), where("uid", "==", uid)));
    if (!qSnap.empty) return qSnap.docs[0].data();
  } catch (e) {
    console.warn("TMC: query user fallback cũng thất bại:", e.code);
  }
  return null;
}

/**
 * FIX-2: Hàm trung tâm để cập nhật user document.
 * Luôn dùng uid làm document ID.
 */
...
async function handleRegister() {
  ...
  try {
    ...
    if (!dupSnap.empty) { showErr("Tên đăng nhập đã tồn tại!"); return; }

    const safeSlug  = username.toLowerCase().replace(/\s+/g, "_");
    ...
    // FIX-1 & FIX-2: Document ID = uid (Firebase Auth UID), luôn nhất quán
    ...
    if      (err.code === "auth/email-already-in-use") showErr("Email hoặc tên này đã được dùng!");
    else if (err.code === "auth/weak-password")        showErr("Mật khẩu quá yếu! Dùng ít nhất 6 ký tự.");
    else if (err.code === "auth/invalid-email")        showErr("Tên chứa ký tự không hợp lệ, hãy dùng chữ thường.");
...
async function handleLogin() {
  ...
  try {
    ...
    if (!usernameInput.includes("@")) {
      // Tra email từ Firestore theo field username
      ...
      if (!snap.empty) {
        ...
        const stored   = userData.email || "";
        // Ưu tiên email thật; nếu không có thì dùng fallback domain mới
        if (stored && stored.includes("@") && !stored.includes("@tmc.internal") && !stored.includes("@tmccaythue.com")) {
          loginEmail = stored;
        } else {
          loginEmail = `${usernameInput.toLowerCase().replace(/\s+/g, "_")}@tmccaythue.com`;
        }
      } else {
        // Thử cả domain cũ lẫn mới
        loginEmail = `${usernameInput.toLowerCase().replace(/\s+/g, "_")}@tmccaythue.com`;
      }
    }
    ...
    else if (err.code === "auth/too-many-requests") showNotif("⏳ Quá nhiều lần thử! Chờ vài phút.", "error");
...
// ── FIX-1: onAuthStateChanged dùng fetchUserData() thay vì getDoc trực tiếp ──
onAuthStateChanged(auth, async firebaseUser => {
  if (firebaseUser) {
    currentUser     = firebaseUser;
    // FIX-1: dùng hàm fetchUserData có fallback query để đảm bảo lấy được data
    currentUserData = await fetchUserData(firebaseUser.uid);
...
async function openOrderModal(svcId) {
  ...
  } else { subWrap.innerHTML = ""; subWrap.style.display = "none"; }
...
async function submitOrder() {
  ...
  if (currentUserData) {
    ...
    if (dInfo.title && dInfo.discount > 0 && finalPrice >= dInfo.title.minOrder) {
      if (dInfo.discount === 100) { finalPrice = 0; discountInfo = "👑 Đại Đế Cảnh — Miễn phí 100%"; }
      else { finalPrice = Math.round(finalPrice * (1 - dInfo.discount / 100)); discountInfo = `${dInfo.title.name} giảm ${dInfo.discount}%`; }
    } else if (dInfo.expired && dInfo.title) {
      showNotif(`⚠️ Danh hiệu ${dInfo.title.name} đã hết hạn!`, "info");
    }
  }

  // FIX-1: Dùng currentUserData đã được fetch đúng – không null nữa
  ...
  try {
    ...
    if (dup) {
      ...
      showNotif(`⏳ Vừa đặt gói này! Chờ ${waitSec}s rồi thử lại.`, "error");
      return;
    }
  } catch { /* không block */ }
  ...
  const order   = {
    ...
    serviceId: currentOrderSvc.id, serviceName: currentOrderSvc.name,
    serviceCode: currentOrderSvc.code, price: finalPrice,
    origPrice: currentOrderSvc.price, discountInfo: discountInfo || null,
  ...
  if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = "⏳ Đang đặt..."; }

  try {
    await setDoc(doc(db, "orders", orderId), order);
    // FIX-2: Dùng updateUserData() – đường dẫn đúng theo uid
    await updateUserData(currentUser.uid, { balance: balance - finalPrice });
    // FIX-3: Re-fetch để sync số dư mới nhất về UI
    ...
    if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = "🎯 Đặt Đơn"; }
  }
}
...
async function loadAndRenderHistory() {
  ...
  try {
    ...
    el.innerHTML = `<div class="no-data"><p>❌ Không tải được: ${e.code || e.message}</p></div>`;
  }
}
...
async function renderProfile() {
  ...
  // FIX-1: Re-fetch mới nhất để đảm bảo số dư và thông tin chính xác
  const freshData = await fetchUserData(currentUser.uid);
  if (freshData) currentUserData = freshData;

  const user = currentUserData;
  if (!user) {
    document.getElementById("profileCard").innerHTML = `<div class="no-data"><p>❌ Không tải được hồ sơ. Vui lòng thử lại.</p></div>`;
    return;
  }

  const ordersSnap = await getDocs(query(
    collection(db, "orders"), where("uid", "==", currentUser.uid), orderBy("timestamp", "desc")
  ));
  ...
  // FIX-3: Số dư hiển thị nổi bật và rõ ràng
  const balance       = user.balance || 0;
  const balanceColor  = balance > 0 ? "var(--green)" : "rgba(240,230,255,0.4)";
  const balanceBorder = balance > 0 ? "rgba(74,222,128,0.4)" : "rgba(255,255,255,0.1)";
  const balanceBg     = balance > 0 ? "rgba(74,222,128,0.08)" : "rgba(255,255,255,0.03)";
  ...
  document.getElementById("profileCard").innerHTML = `
    ...
    <!-- FIX-3: Số dư nổi bật, full-width, dễ nhìn -->
    <div style="margin:14px 0 10px;padding:14px 20px;border-radius:14px;background:${balanceBg};border:1.5px solid ${balanceBorder};text-align:center">
      <div style="font-size:0.72rem;color:rgba(240,230,255,0.5);letter-spacing:0.08em;margin-bottom:4px">💰 SỐ DƯ TÀI KHOẢN</div>
      <div style="font-family:'Orbitron',sans-serif;font-size:1.6rem;font-weight:700;color:${balanceColor};line-height:1.2">${formatPrice(balance)}</div>
      ${balance === 0 ? `<div style="font-size:0.7rem;color:rgba(240,230,255,0.35);margin-top:4px">Nạp thẻ hoặc liên hệ shop để nạp tiền</div>` : ""}
    </div>
...
function renderProfileExpiryBox(user) {
  ...
  let boxClass = "title-expiry-box", countdownHtml = "", renewHtml = "";
  if (remain > 0) {
    if (remain <= DAY3) {
      boxClass += " title-expiry-urgent";
      countdownHtml = `<div class="expiry-countdown expiry-urgent-text">⚠️ Còn ${days} ngày hết hạn!</div>`;
      renewHtml     = `<div style="font-size:0.72rem;color:rgba(240,230,255,.5);margin-bottom:8px">Gia hạn giá <strong style="color:#ff9040">50%</strong> = ${formatPrice(renewPrice50)}</div><button class="btn-renew btn-renew-normal" onclick="openBuyTitle('${t.id}')">🔄 Gia Hạn Ngay</button>`;
    } else {
    ...
    if (-remain <= DAY3) {
      ...
      renewHtml     = `<div style="font-size:0.72rem;color:rgba(240,230,255,.5);margin-bottom:8px">Gia hạn cửa sổ 3 ngày giá <strong style="color:#ff9040">50%</strong> = ${formatPrice(renewPrice50)}</div><button class="btn-renew btn-renew-normal" onclick="openBuyTitle('${t.id}')">🔄 Gia Hạn (50%)</button>`;
    } else {
      ...
      renewHtml     = `<div style="font-size:0.72rem;color:rgba(240,230,255,.5);margin-bottom:8px">Phải mua lại giá gốc ${formatPrice(t.price)}</div><button class="btn-renew btn-renew-full" onclick="openBuyTitle('${t.id}')">🛒 Mua Lại Full Giá</button>`;
    }
  }
  expiryEl.innerHTML = `<div class="${boxClass}">
    ...
    ${countdownHtml}<div style="margin-top:10px">${renewHtml}</div>
  </div>`;
}
...
async function openBuyTitle(titleId) {
  ...
  try {
    ...
    // FIX-3: Re-fetch để sync số dư mới
    currentUserData = await fetchUserData(currentUser.uid);
    showNotif(`✅ Đã ${ri.type === "new" ? "mua" : "gia hạn"} ${t.name}! Chờ chủ shop cấp.`, "success");
    triggerCelebration();
  } catch (e) { showNotif("❌ Lỗi mua danh hiệu: " + (e.message || "thử lại!"), "error"); }
}
    ...
    _orderListener = onSnapshot(
      ...
      snapshot => {
        snapshot.docChanges().forEach(change => {
          ...
          if (change.type === "modified" && _knownOrderStatus[order.id] === "pending" && order.status === "done") {
            showToastDone(order);
            triggerCelebration();
          }
...
function startFruitStockListener() {
  ...
  _fruitStockListener = onSnapshot(collection(db, "fruitstock"), snap => {
    ...
    snap.docChanges().forEach(change => { if (change.type === "removed") delete _fruitStockCache[change.doc.id]; });
    applyFruitStockToUI();
  }, () => {});
}
...
function applyFruitStockToUI() {
  ...
  fruits.forEach(svc => {
    ...
    if (badge) { badge.className = `fruit-stock-badge ${inStock ? "in-stock" : "out-of-stock"}`; badge.innerHTML = inStock ? `✅ Còn ${qty}` : "❌ Hết"; }
    const btn = document.getElementById(`svc-btn-${svc.id}`);
    if (btn) {
      if (!inStock) { btn.disabled = true; btn.textContent = "Hết Hàng"; btn.style.background = "rgba(255,255,255,0.08)"; btn.style.color = "rgba(240,230,255,0.3)"; btn.style.cursor = "not-allowed"; }
  ...
  if (catBadge) {
    ...
    const cnt   = fruits.filter(s => getFruitStock(s.id).inStock).length;
    if (total > 0) {
      if      (cnt === 0)     { catBadge.innerHTML = "❌ Hết hàng"; catBadge.style.color = "var(--red)"; }
      else if (cnt < total)   { catBadge.innerHTML = `🍎 Còn ${cnt}/${total} loại`; catBadge.style.color = "#ffa502"; }
...
function updateFruitPanelHeader() {
  ...
  const color = cnt === 0 ? "var(--red)" : cnt < total / 2 ? "#ffa502" : "var(--green)";
  el.innerHTML = `<div class="fruit-stock-banner">
    <div class="fruit-stock-banner-dot" style="background:${color}"></div>
    <div style="flex:1">
      <div style="font-weight:700;font-size:0.85rem;color:${color}">🍎 Tồn kho: ${cnt}/${total} loại còn hàng</div>
...
// ===== DANH HIỆU GRID (SHOP) ================================
// ============================================================
function titleLogo(t) { return DH_LOGOS[t.id] || `<span style="font-size:2rem">${t.icon}</span>`; }
...
function renderTitleShopGrid() {
  ...
  const T     = TITLES;
  const cardHtml = (t, tierCls, extraCls = "") => {
    const isFree    = t.free || t.price === 0;
    const renewHint = !isFree ? `<div style="font-size:0.62rem;color:rgba(240,230,255,0.3);margin-bottom:6px">GH sớm: <span style="color:var(--accent)">${formatPrice(Math.round(t.price * 0.4))}</span> · Hết ≤3 ngày: <span style="color:var(--gold)">${formatPrice(Math.round(t.price * 0.5))}</span></div>` : "";
    return `<div class="dh-card ${tierCls} ${extraCls}" onclick="openBuyTitle('${t.id}')">
  ...
  row3.innerHTML = `<div class="dh-card tier-8 banner-card" onclick="openBuyTitle('${doKiep.id}')">
    <div class="dh-card-logo" style="width:72px;height:72px;flex-shrink:0;margin:0">${titleLogo(doKiep)}</div>
    <div class="dh-card-body">
      ...
      <div class="dh-card-perk" style="font-size:0.72rem">${doKiep.desc}<br><span class="hl">✨ Giảm ${doKiep.discount}% đơn ≥${formatPrice(doKiep.minOrder)}</span></div>
    </div>
...
window.applyVideoSetting = applyVideoSetting;
// admin.js dùng để đọc services/categories/settings
...
// admin.js dùng để ghi user data đúng path
