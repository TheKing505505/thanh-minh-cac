...
// Cache-bust "?v=2" để đảm bảo GitHub Pages/browser không phục vụ bản cache cũ
// của title-logos.js (file này có thể từng thiếu "export" ở các lần build trước)
...
// ── Cấu hình shop realtime từ Firestore (system_configs) ──────────────
// Cho phép khách vãng lai (chưa đăng nhập) cũng thấy thay đổi của admin ngay lập tức
let _configListener = null;
const _configCache  = { services: null, categories: null, settings: {} };
...
// ============================================================
// ===== SERVICES – Firestore (system_configs) realtime ======
// Khách vãng lai (chưa đăng nhập) vẫn thấy thay đổi của admin
// ============================================================
function startConfigListener() {
  if (_configListener) return; // đã chạy rồi, không khởi động lại
  _configListener = onSnapshot(
    ...
    snap => {
      ...
      // Đồng bộ xuống localStorage để dùng làm cache offline
      if (_configCache.services)   localStorage.setItem("tmc_services",   JSON.stringify(_configCache.services));
      if (_configCache.categories) localStorage.setItem("tmc_categories", JSON.stringify(_configCache.categories));
      renderServices();
      applyVideoSetting();
    },
    err => console.warn("TMC configListener:", err.code || err.message)
  );
}

function getServices() {
  // 1) Ưu tiên dữ liệu realtime từ Firestore (luôn mới nhất cho mọi người, kể cả khách vãng lai)
  if (_configCache.services && _configCache.services.length) return _configCache.services;
  // 2) Fallback localStorage (khi mất mạng hoặc Firestore chưa kịp tải)
...
/** Lưu services – ghi cả Firestore (nguồn chính) và localStorage (cache) */
async function saveServices(svcs) {
  localStorage.setItem("tmc_services", JSON.stringify(svcs));
  _configCache.services = svcs;
  try { await setDoc(doc(db, "system_configs", "services"), { list: svcs }); }
  catch (e) { console.warn("TMC saveServices Firestore:", e.code || e.message); }
}

function getCustomCategories() {
  if (_configCache.categories && _configCache.categories.length) return _configCache.categories;
...
/** Lưu categories – ghi cả Firestore và localStorage */
async function saveCustomCategories(cats) {
  localStorage.setItem("tmc_categories", JSON.stringify(cats));
  _configCache.categories = cats;
  try { await setDoc(doc(db, "system_configs", "categories"), { list: cats }); }
  catch (e) { console.warn("TMC saveCustomCategories Firestore:", e.code || e.message); }
}
...
/** Lưu settings (video, thông báo) – ghi cả Firestore và localStorage */
async function saveShopSettings(s) {
  localStorage.setItem("tmc_settings", JSON.stringify(s));
  _configCache.settings = s;
  try { await setDoc(doc(db, "system_configs", "settings"), s); }
  catch (e) { console.warn("TMC saveShopSettings Firestore:", e.code || e.message); }
}
...
onAuthStateChanged(auth, async firebaseUser => {
  if (firebaseUser) {
    ...
    try {
      ...
      if (!snap.exists()) console.warn("TMC: user document chưa tồn tại cho uid", firebaseUser.uid);
    } catch (e) {
      console.error("TMC onAuthStateChanged fetchUser:", e.code || e.message);
      currentUserData = null;
    }
    updateNav();
...
async function updateNav() {
  ...
  if (user) {
    ...
    const safeChar = displayName && displayName.length ? displayName[0].toUpperCase() : "?";
    const avEl = document.getElementById("navAvatar");
    const unEl = document.getElementById("navUsername");
    if (avEl) avEl.textContent = safeChar;
...
async function loadAndRenderHistory() {
  ...
  try {
    ...
    el.innerHTML = `<div class="no-data"><p>❌ Không tải được lịch sử: ${e.code || e.message}</p><p style="font-size:0.72rem;margin-top:6px;color:rgba(240,230,255,0.4)">Kiểm tra Firestore Rules collection "orders"</p></div>`;
  }
}
...
async function renderProfile() {
  ...
  // Nếu currentUserData chưa kịp tải (do timing với onAuthStateChanged) → thử tải lại
  if (!currentUserData) {
    try {
      const snap = await getDoc(doc(db, "users", currentUser.uid));
      currentUserData = snap.exists() ? snap.data() : null;
    } catch (e) {
      console.error("renderProfile re-fetch user:", e);
    }
  }
  ...
  try {
    ...
    console.error("renderProfile orders:", e);
    // Vẫn tiếp tục render hồ sơ dù không lấy được đơn hàng
  }
...
// Dùng chung với admin.js (tránh logic getServices bị lệch + đảm bảo admin
// ghi dữ liệu vào Firestore system_configs, không chỉ localStorage riêng máy)
...
window.__tmcSaveShopSettings     = saveShopSettings;

// ============================================================
// ===== INIT =================================================
// ============================================================
(function init() {
  // Khởi động listener config TRƯỚC để khách vãng lai cũng nhận thay đổi của admin ngay
  startConfigListener();
  renderServices();
  applyVideoSetting();
  loadTopNap();
